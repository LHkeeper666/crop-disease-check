# 数据库设计文档

## 智慧农业病虫害无人值守监测与决策系统

---

| 项目 | 内容 |
|------|------|
| 文档版本 | V2.1 |
| 编写日期 | 2026-06-10 |
| 数据库 | MySQL 5.7+ / 8.0 |
| 字符集 | utf8mb4 |

---

## ⚠️ V2.1 查漏补缺记录（基于前端功能反推）

> 以下内容为对照前端全部页面功能后补充的缺失表和字段，后端成员请重点关注。

### 新增表（3张）

| 表名 | 说明 | 原因 |
|------|------|------|
| `company` | 企业/租户表 | 前端有邀请码加入企业流程（PendingView），UserInfo 有 companyId，需要企业表支撑多租户 |
| `greenhouse` | 温室/大棚表 | grid 表已有 `greenhouse_id` 外键但目标表不存在；前端 Dashboard 展示温室元数据（区域编号、作物、定植日期等） |
| `environment_data` | 环境监测数据表 | 前端 Dashboard 实时展示空气温度、土壤湿度、光照、CO₂、pH、EC、N/P/K、能耗等，无表存储 |

### 现有表补充字段

| 表名 | 新增字段 | 原因 |
|------|----------|------|
| `sys_user` | `company_id`, `avatar`, `approved` | 前端 UserInfo 有 companyId（多租户隔离）、avatar（头像）、approved（审核状态） |
| `camera` | `rtsp_url_sub`, `capture_resolution`, `capture_quality`, `reconnect_interval` | 接口文档 4.2/4.3 定义了这些字段但数据库未建 |
| `work_order` | `type`, `grid_label`, `pest_name`, `confidence`, `created_by` | 前端工单列表直接展示 type(disease/pest)、gridLabel、pestName、confidence，当前表仅通过 inference_id 间接关联，查询不便 |
| `daily_report` | `detections`, `disease_count`, `pest_count`, `handled_rate`, `greenhouse_id`, `company_id` | 前端日报表格展示这些字段，当前表仅有 summary_json |
| `grid` | `score`, `company_id` | 前端热力图按网格展示风险评分 score，需要多租户隔离 |
| `camera` | `company_id` | 多租户隔离 |
| `inspection_plan` | `company_id` | 多租户隔离 |
| `daily_report` | `company_id` | 多租户隔离 |

---

## 目录

1. [表结构总览](#1-表结构总览)
2. [建表SQL](#2-建表sql)
3. [初始数据SQL](#3-初始数据sql)
4. [ER关系说明](#4-er关系说明)

---

## 1. 表结构总览

| 模块 | 表名 | 说明 |
|------|------|------|
| 企业 | company | 企业/租户表（多租户） |
| 用户 | sys_user | 用户表(ADMIN/EXPERT/MANAGER/VISITOR) |
| 温室 | greenhouse | 温室/大棚元数据 |
| 摄像头 | camera | 摄像头管理 |
| | camera_grid | 摄像头覆盖网格关联 |
| 网格 | grid | 网格/地块区域 |
| 环境 | environment_data | 环境监测实时数据 |
| 病虫害 | pest_info | 病虫害知识库 |
| 上报识别 | report | 图像上报记录 |
| | inference | 识别结果 |
| | audit_record | 审核记录 |
| 防治方案 | prevention_plan | 防治方案 |
| | prevention_plan_version | 方案版本历史 |
| 工单 | work_order | 工单管理 |
| | work_order_history | 工单状态历史 |
| 巡检 | inspection_plan | 巡检计划 |
| | inspection_camera | 巡检关联摄像头 |
| | inspection_log | 巡检日志 |
| 报告 | daily_report | 日度报告 |
| 农业大脑 | ai_conversation | 对话 |
| | ai_message | 消息 |
| 系统 | sys_log | 操作日志 |

---

## 2. 建表SQL

### 2.1 企业/租户表 (company)

```sql
CREATE TABLE company (
    id            VARCHAR(36) PRIMARY KEY COMMENT '企业UUID',
    name          VARCHAR(128) NOT NULL COMMENT '企业名称',
    invite_code   VARCHAR(20) NOT NULL UNIQUE COMMENT '邀请码',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted       TINYINT DEFAULT 0 COMMENT '逻辑删除'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='企业/租户表';
```

### 2.2 用户表 (sys_user)

```sql
CREATE TABLE sys_user (
    id            VARCHAR(36) PRIMARY KEY COMMENT '用户UUID',
    username      VARCHAR(64) NOT NULL UNIQUE COMMENT '用户名',
    password      VARCHAR(128) NOT NULL COMMENT '密码(BCrypt)',
    name          VARCHAR(64) COMMENT '真实姓名',
    role          VARCHAR(20) NOT NULL DEFAULT 'VISITOR' COMMENT '角色: ADMIN/EXPERT/MANAGER/VISITOR',
    phone         VARCHAR(20) COMMENT '手机号',
    email         VARCHAR(128) COMMENT '邮箱',
    avatar        VARCHAR(255) COMMENT '头像URL',
    company_id    VARCHAR(36) COMMENT '所属企业ID',
    approved      TINYINT DEFAULT 0 COMMENT '是否审核通过: 0=待审核 1=已通过',
    status        VARCHAR(20) DEFAULT 'ACTIVE' COMMENT '状态: ACTIVE/DISABLED',
    last_login_at DATETIME COMMENT '最后登录时间',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted       TINYINT DEFAULT 0 COMMENT '逻辑删除',
    INDEX idx_role (role),
    INDEX idx_status (status),
    INDEX idx_company (company_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户表';
```

### 2.3 温室/大棚表 (greenhouse)

```sql
CREATE TABLE greenhouse (
    id            VARCHAR(36) PRIMARY KEY COMMENT '温室UUID',
    sector_id     VARCHAR(20) NOT NULL COMMENT '区域编号(如GH-A1)',
    crop_species  VARCHAR(128) COMMENT '作物种类(拉丁学名)',
    planting_date DATE COMMENT '定植日期',
    location      VARCHAR(50) COMMENT '经纬度坐标(如34.2614N, 108.9423E)',
    area          VARCHAR(20) COMMENT '面积(如2400 m²)',
    status        VARCHAR(20) DEFAULT 'ACTIVE' COMMENT '状态: ACTIVE/MAINTENANCE/INACTIVE',
    company_id    VARCHAR(36) COMMENT '所属企业ID',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted       TINYINT DEFAULT 0 COMMENT '逻辑删除',
    INDEX idx_company (company_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='温室/大棚表';
```

### 2.4 摄像头表 (camera)

```sql
CREATE TABLE camera (
    id                  VARCHAR(36) PRIMARY KEY COMMENT '摄像头UUID',
    name                VARCHAR(128) NOT NULL COMMENT '摄像头名称',
    rtsp_url            VARCHAR(512) COMMENT 'RTSP主码流地址',
    rtsp_url_sub        VARCHAR(512) COMMENT 'RTSP子码流地址(低分辨率预览)',
    location_x          DECIMAL(10,6) COMMENT '经度',
    location_y          DECIMAL(10,6) COMMENT '纬度',
    direction           DECIMAL(5,1) COMMENT '朝向角度',
    capture_resolution  VARCHAR(20) DEFAULT '640x640' COMMENT '抓拍分辨率',
    capture_quality     INT DEFAULT 85 COMMENT '抓拍JPEG质量(1-100)',
    reconnect_interval  INT DEFAULT 30 COMMENT '断流重连间隔(秒)',
    status              VARCHAR(20) DEFAULT 'OFFLINE' COMMENT '状态: ONLINE/OFFLINE/FAULT',
    last_frame_at       DATETIME COMMENT '最后抓拍时间',
    company_id          VARCHAR(36) COMMENT '所属企业ID',
    created_at          DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at          DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted             TINYINT DEFAULT 0 COMMENT '逻辑删除',
    INDEX idx_company (company_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='摄像头表';
```

### 2.3 摄像头覆盖网格关联表 (camera_grid)

```sql
CREATE TABLE camera_grid (
    id          BIGINT PRIMARY KEY AUTO_INCREMENT,
    camera_id   VARCHAR(36) NOT NULL COMMENT '摄像头ID',
    grid_id     VARCHAR(36) NOT NULL COMMENT '网格ID',
    UNIQUE KEY uk_camera_grid (camera_id, grid_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='摄像头覆盖网格关联';
```

### 2.4 网格区域表 (grid)

```sql
CREATE TABLE grid (
    id              VARCHAR(36) PRIMARY KEY COMMENT '网格UUID',
    label           VARCHAR(32) NOT NULL COMMENT '网格编号(A1/B3等)',
    greenhouse_id   VARCHAR(36) COMMENT '大棚ID',
    polygon_coords  JSON COMMENT '多边形坐标点',
    area_m2         DECIMAL(10,2) COMMENT '面积(平方米)',
    crop_type       VARCHAR(64) COMMENT '作物类型',
    score           DECIMAL(3,2) DEFAULT 0.00 COMMENT '当前风险评分(0.00-1.00)',
    company_id      VARCHAR(36) COMMENT '所属企业ID',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at      DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted         TINYINT DEFAULT 0 COMMENT '逻辑删除',
    INDEX idx_label (label),
    INDEX idx_company (company_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='网格区域表';
```

### 2.8 环境监测数据表 (environment_data)

```sql
CREATE TABLE environment_data (
    id              VARCHAR(36) PRIMARY KEY COMMENT '记录UUID',
    greenhouse_id   VARCHAR(36) NOT NULL COMMENT '关联温室ID',
    air_temp        DECIMAL(5,1) COMMENT '空气温度(°C)',
    soil_moisture   DECIMAL(5,1) COMMENT '土壤湿度(%)',
    humidity        DECIMAL(5,1) COMMENT '空气湿度(%)',
    light_level     INT COMMENT '光照强度(lux)',
    co2             INT COMMENT 'CO₂浓度(ppm)',
    soil_ph         DECIMAL(3,1) COMMENT '土壤pH',
    ec              DECIMAL(4,2) COMMENT '电导率(mS/cm)',
    nitrogen        INT COMMENT '氮(mg/kg)',
    phosphorus      INT COMMENT '磷(mg/kg)',
    potassium       INT COMMENT '钾(mg/kg)',
    energy_current  DECIMAL(6,2) COMMENT '当前功耗(Kw)',
    energy_max      DECIMAL(6,2) COMMENT '最大负载(Kw)',
    recorded_at     DATETIME NOT NULL COMMENT '记录时间',
    company_id      VARCHAR(36) COMMENT '所属企业ID',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_greenhouse (greenhouse_id),
    INDEX idx_recorded (recorded_at),
    INDEX idx_company (company_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='环境监测数据表';
```

### 2.9 病虫害信息表 (pest_info)

```sql
CREATE TABLE pest_info (
    id          VARCHAR(36) PRIMARY KEY COMMENT '病虫害UUID',
    pest_name   VARCHAR(128) NOT NULL COMMENT '病虫害名称',
    pest_type   VARCHAR(20) NOT NULL COMMENT '类型: DISEASE/PEST/WEED',
    description TEXT COMMENT '描述',
    conditions  TEXT COMMENT '发生条件',
    prevention  TEXT COMMENT '防治方法',
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at  DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted     TINYINT DEFAULT 0 COMMENT '逻辑删除'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='病虫害知识库';
```

### 2.6 图像上报记录表 (report)

```sql
CREATE TABLE report (
    id            VARCHAR(36) PRIMARY KEY COMMENT '上报UUID',
    user_id       VARCHAR(36) NOT NULL COMMENT '上报用户ID',
    grid_id       VARCHAR(36) NOT NULL COMMENT '网格/地块ID',
    crop_type     VARCHAR(64) COMMENT '农作物品种',
    image_urls    JSON COMMENT '图片URL数组',
    found_at      DATETIME NOT NULL COMMENT '发现时间',
    description   VARCHAR(500) COMMENT '补充描述',
    status        VARCHAR(30) DEFAULT 'PENDING_RECOGNITION' COMMENT '状态: PENDING_RECOGNITION/PENDING/AUDITED/REJECTED',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted       TINYINT DEFAULT 0 COMMENT '逻辑删除',
    INDEX idx_user (user_id),
    INDEX idx_grid (grid_id),
    INDEX idx_status (status),
    INDEX idx_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='图像上报记录表';
```

### 2.7 识别结果表 (inference)

```sql
CREATE TABLE inference (
    id                VARCHAR(36) PRIMARY KEY COMMENT '识别UUID',
    report_id         VARCHAR(36) NOT NULL COMMENT '关联上报ID',
    pest_id           VARCHAR(36) COMMENT '关联病虫害ID',
    pest_name         VARCHAR(128) COMMENT '识别的病虫害名称',
    confidence        DECIMAL(5,4) COMMENT '置信度(0-1)',
    is_low_confidence TINYINT DEFAULT 0 COMMENT '是否低置信度(<0.6)',
    pipeline          VARCHAR(20) COMMENT '识别管道: DISEASE/PEST',
    bbox              JSON COMMENT '检测框坐标',
    created_at        DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_report (report_id),
    INDEX idx_confidence (confidence)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='识别结果表';
```

### 2.8 审核记录表 (audit_record)

```sql
CREATE TABLE audit_record (
    id            VARCHAR(36) PRIMARY KEY COMMENT '审核UUID',
    report_id     VARCHAR(36) NOT NULL COMMENT '关联上报ID',
    auditor_id    VARCHAR(36) NOT NULL COMMENT '审核人ID',
    audit_result  VARCHAR(20) NOT NULL COMMENT '审核结果: APPROVED/REJECTED',
    comment       VARCHAR(500) COMMENT '审核意见',
    audited_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '审核时间',
    INDEX idx_report (report_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='审核记录表';
```

### 2.9 防治方案表 (prevention_plan)

```sql
CREATE TABLE prevention_plan (
    id            VARCHAR(36) PRIMARY KEY COMMENT '方案UUID',
    report_id     VARCHAR(36) NOT NULL COMMENT '关联上报ID',
    content       TEXT NOT NULL COMMENT '方案内容',
    suggest_time  DATE COMMENT '建议执行时间',
    author_id     VARCHAR(36) COMMENT '制定人ID',
    version       INT DEFAULT 1 COMMENT '版本号',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    UNIQUE KEY uk_report (report_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='防治方案表';
```

### 2.10 防治方案版本历史表 (prevention_plan_version)

```sql
CREATE TABLE prevention_plan_version (
    id            BIGINT PRIMARY KEY AUTO_INCREMENT,
    plan_id       VARCHAR(36) NOT NULL COMMENT '方案ID',
    content       TEXT NOT NULL COMMENT '方案内容',
    suggest_time  DATE COMMENT '建议执行时间',
    version       INT NOT NULL COMMENT '版本号',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_plan (plan_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='防治方案版本历史';
```

### 2.11 工单表 (work_order)

```sql
CREATE TABLE work_order (
    id              VARCHAR(36) PRIMARY KEY COMMENT '工单UUID',
    title           VARCHAR(256) NOT NULL COMMENT '工单标题',
    severity        VARCHAR(20) NOT NULL COMMENT '严重程度: LOW/MEDIUM/HIGH/CRITICAL',
    status          VARCHAR(20) DEFAULT 'PENDING' COMMENT '状态: PENDING/PROCESSING/DONE/IGNORED/ESCALATED',
    type            VARCHAR(20) COMMENT '类型: disease/pest',
    grid_label      VARCHAR(10) COMMENT '关联网格编号(如A1/B3)',
    pest_name       VARCHAR(50) COMMENT '病虫害名称',
    confidence      DECIMAL(3,2) COMMENT '检测置信度(0.00-1.00)',
    inference_id    VARCHAR(36) COMMENT '关联识别ID',
    assigned_to     VARCHAR(36) COMMENT '指派给用户ID',
    created_by      VARCHAR(36) COMMENT '创建人ID',
    expert_comment  VARCHAR(500) COMMENT '专家备注',
    callback_token  VARCHAR(128) COMMENT '回调Token',
    token_expire_at DATETIME COMMENT 'Token过期时间',
    token_used      TINYINT DEFAULT 0 COMMENT 'Token是否已使用',
    company_id      VARCHAR(36) COMMENT '所属企业ID',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at      DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX idx_status (status),
    INDEX idx_severity (severity),
    INDEX idx_assigned (assigned_to),
    INDEX idx_token (callback_token),
    INDEX idx_company (company_id),
    INDEX idx_grid (grid_label)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='工单表';
```

### 2.12 工单状态历史表 (work_order_history)

```sql
CREATE TABLE work_order_history (
    id            BIGINT PRIMARY KEY AUTO_INCREMENT,
    workorder_id  VARCHAR(36) NOT NULL COMMENT '工单ID',
    status        VARCHAR(20) NOT NULL COMMENT '状态',
    operator_id   VARCHAR(36) COMMENT '操作人ID',
    operator_name VARCHAR(64) COMMENT '操作人名称',
    comment       VARCHAR(500) COMMENT '备注',
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_workorder (workorder_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='工单状态历史';
```

### 2.13 巡检计划表 (inspection_plan)

```sql
CREATE TABLE inspection_plan (
    id                  VARCHAR(36) PRIMARY KEY COMMENT '计划UUID',
    name                VARCHAR(128) NOT NULL COMMENT '计划名称',
    cron_expression     VARCHAR(64) COMMENT 'Cron表达式',
    active_hours_start  VARCHAR(10) COMMENT '生效开始时间(HH:mm)',
    active_hours_end    VARCHAR(10) COMMENT '生效结束时间(HH:mm)',
    is_active           TINYINT DEFAULT 1 COMMENT '是否启用',
    company_id          VARCHAR(36) COMMENT '所属企业ID',
    created_at          DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at          DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='巡检计划表';
```

### 2.14 巡检计划摄像头关联表 (inspection_camera)

```sql
CREATE TABLE inspection_camera (
    id          BIGINT PRIMARY KEY AUTO_INCREMENT,
    plan_id     VARCHAR(36) NOT NULL COMMENT '计划ID',
    camera_id   VARCHAR(36) NOT NULL COMMENT '摄像头ID',
    UNIQUE KEY uk_plan_camera (plan_id, camera_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='巡检计划摄像头关联';
```

### 2.15 巡检日志表 (inspection_log)

```sql
CREATE TABLE inspection_log (
    id              VARCHAR(36) PRIMARY KEY COMMENT '日志UUID',
    plan_id         VARCHAR(36) COMMENT '计划ID',
    camera_id       VARCHAR(36) NOT NULL COMMENT '摄像头ID',
    capture_time    DATETIME NOT NULL COMMENT '抓拍时间',
    image_url       VARCHAR(512) COMMENT '图片URL',
    disease_count   INT DEFAULT 0 COMMENT '病害数量',
    pest_count      INT DEFAULT 0 COMMENT '虫害数量',
    max_confidence  DECIMAL(5,4) COMMENT '最高置信度',
    duration_ms     INT COMMENT '耗时(毫秒)',
    status          VARCHAR(20) DEFAULT 'SUCCESS' COMMENT '状态: SUCCESS/FAILED',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_camera (camera_id),
    INDEX idx_capture_time (capture_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='巡检日志表';
```

### 2.16 日度报告表 (daily_report)

```sql
CREATE TABLE daily_report (
    id              VARCHAR(36) PRIMARY KEY COMMENT '报告UUID',
    report_date     DATE NOT NULL COMMENT '报告日期',
    detections      INT DEFAULT 0 COMMENT '识别总次数',
    disease_count   INT DEFAULT 0 COMMENT '病害次数',
    pest_count      INT DEFAULT 0 COMMENT '虫害次数',
    handled_rate    DECIMAL(3,2) DEFAULT 0.00 COMMENT '处理率(0.00-1.00)',
    greenhouse_id   VARCHAR(36) COMMENT '关联温室ID',
    summary_json    JSON COMMENT '统计数据JSON',
    html_content    LONGTEXT COMMENT '报告HTML内容(AI生成)',
    email_sent      TINYINT DEFAULT 0 COMMENT '是否已发送邮件',
    email_sent_at   DATETIME COMMENT '邮件发送时间',
    company_id      VARCHAR(36) COMMENT '所属企业ID',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    UNIQUE KEY uk_date (report_date),
    INDEX idx_company (company_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='日度报告表';
```

### 2.17 农业大脑对话表 (ai_conversation)

```sql
CREATE TABLE ai_conversation (
    id          VARCHAR(36) PRIMARY KEY COMMENT '对话UUID',
    user_id     VARCHAR(36) NOT NULL COMMENT '用户ID',
    title       VARCHAR(256) COMMENT '对话标题',
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at  DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX idx_user (user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='农业大脑对话表';
```

### 2.18 农业大脑消息表 (ai_message)

```sql
CREATE TABLE ai_message (
    id              VARCHAR(36) PRIMARY KEY COMMENT '消息UUID',
    conversation_id VARCHAR(36) NOT NULL COMMENT '对话ID',
    user_id         VARCHAR(36) NOT NULL COMMENT '用户ID',
    role            VARCHAR(20) NOT NULL COMMENT '角色: USER/ASSISTANT',
    content         TEXT NOT NULL COMMENT '消息内容',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_conversation (conversation_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='农业大脑消息表';
```

### 2.19 操作日志表 (sys_log)

```sql
CREATE TABLE sys_log (
    id          BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '日志ID',
    user_id     VARCHAR(36) COMMENT '操作用户ID',
    username    VARCHAR(64) COMMENT '用户名',
    operation   VARCHAR(64) COMMENT '操作类型',
    method      VARCHAR(256) COMMENT '请求方法',
    params      TEXT COMMENT '请求参数',
    ip          VARCHAR(64) COMMENT 'IP地址',
    duration    BIGINT COMMENT '执行时长(ms)',
    status      TINYINT DEFAULT 1 COMMENT '状态: 0=失败 1=成功',
    error_msg   TEXT COMMENT '错误信息',
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_user (user_id),
    INDEX idx_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='操作日志表';
```

---

## 3. 初始数据SQL

### 3.1 企业数据

```sql
INSERT INTO company (id, name, invite_code) VALUES
('company-001', 'TreeForge 智慧农场', 'TF2026'),
('company-002', '绿丰农业科技', 'AG2026');
```

### 3.2 用户数据

```sql
-- 管理员账号 (密码: admin123)
INSERT INTO sys_user (id, username, password, name, role, email, company_id, approved, status) VALUES
('u001', 'admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EH', '系统管理员', 'ADMIN', 'admin@treeforge.cn', 'company-001', 1, 'ACTIVE');

-- 示例专家账号
INSERT INTO sys_user (id, username, password, name, role, phone, email, company_id, approved, status) VALUES
('u002', 'expert_li', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EH', '李专家', 'EXPERT', '13900139000', '2043412933@qq.com', 'company-001', 1, 'ACTIVE'),
('u003', 'expert_wang', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EH', '王专家', 'EXPERT', '13700137000', '1367858987@qq.com', 'company-001', 1, 'ACTIVE');

-- 示例管理者账号
INSERT INTO sys_user (id, username, password, name, role, phone, email, company_id, approved, status) VALUES
('u004', 'manager_zhang', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EH', '张三', 'MANAGER', '13600136000', 'zhang@treeforge.cn', 'company-001', 1, 'ACTIVE');
```

### 3.2 网格区域数据

```sql
INSERT INTO grid (id, label, greenhouse_id, polygon_coords, area_m2, crop_type) VALUES
('g001', 'A1', 'gh001', '[{"x":0,"y":0},{"x":100,"y":0},{"x":100,"y":100},{"x":0,"y":100}]', 100.00, '番茄'),
('g002', 'A2', 'gh001', '[{"x":100,"y":0},{"x":200,"y":0},{"x":200,"y":100},{"x":100,"y":100}]', 100.00, '番茄'),
('g003', 'B1', 'gh002', '[{"x":0,"y":100},{"x":100,"y":100},{"x":100,"y":200},{"x":0,"y":200}]', 100.00, '黄瓜'),
('g004', 'B2', 'gh002', '[{"x":100,"y":100},{"x":200,"y":100},{"x":200,"y":200},{"x":100,"y":200}]', 100.00, '黄瓜'),
('g005', 'B3', 'gh002', '[{"x":200,"y":100},{"x":300,"y":100},{"x":300,"y":200},{"x":200,"y":200}]', 100.00, '番茄');
```

### 3.3 温室数据

```sql
INSERT INTO greenhouse (id, sector_id, crop_species, planting_date, location, area, status, company_id) VALUES
('gh001', 'GH-A1', 'Solanum lycopersicum', '2026-03-15', '34.2614N, 108.9423E', '2400 m²', 'ACTIVE', 'company-001'),
('gh002', 'GH-B1', 'Cucumis sativus', '2026-04-01', '34.2615N, 108.9424E', '1800 m²', 'ACTIVE', 'company-001');
```

### 3.4 摄像头数据

```sql
INSERT INTO camera (id, name, rtsp_url, rtsp_url_sub, location_x, location_y, direction, capture_resolution, capture_quality, reconnect_interval, status, company_id) VALUES
('c001', 'A区-1号摄像头', 'rtsp://192.168.1.101:554/stream1', 'rtsp://192.168.1.101:554/stream2', 120.500000, 35.200000, 90.0, '640x640', 85, 30, 'ONLINE', 'company-001'),
('c002', 'A区-2号摄像头', 'rtsp://192.168.1.102:554/stream1', NULL, 120.500100, 35.200100, 180.0, '640x640', 85, 30, 'ONLINE', 'company-001'),
('c003', 'B区-1号摄像头', 'rtsp://192.168.1.103:554/stream1', 'rtsp://192.168.1.103:554/stream2', 120.500200, 35.200200, 270.0, '640x640', 85, 30, 'ONLINE', 'company-001'),
('c004', 'B区-2号摄像头', 'rtsp://192.168.1.104:554/stream1', NULL, 120.500300, 35.200300, 0.0, '640x640', 85, 30, 'OFFLINE', 'company-001');

-- 摄像头覆盖网格关联
INSERT INTO camera_grid (camera_id, grid_id) VALUES
('c001', 'g001'), ('c001', 'g002'), ('c002', 'g002'),
('c003', 'g003'), ('c003', 'g004'), ('c004', 'g005');
```

### 3.4 病虫害知识库数据

```sql
INSERT INTO pest_info (id, pest_name, pest_type, description, conditions, prevention) VALUES
('p001', '番茄晚疫病', 'DISEASE', '由致病疫霉引起的真菌性病害，主要危害叶片和果实', '高湿低温环境易发，温度15-25℃，相对湿度>75%', '喷洒甲霜灵锰锌，及时通风降湿，摘除病叶'),
('p002', '番茄叶霉病', 'DISEASE', '由番茄叶霉菌引起的真菌性病害，主要危害叶片', '温暖高湿环境，温度20-25℃，相对湿度>80%', '喷洒多菌灵，增加通风，降低种植密度'),
('p003', '番茄蚜虫', 'PEST', '成虫和若虫吸食叶片汁液，传播病毒病', '温暖干燥环境，温度18-25℃', '黄板诱杀，释放瓢虫，喷洒吡虫啉'),
('p004', '红蜘蛛', 'PEST', '叶螨科害虫，吸食叶片汁液导致叶片失绿', '高温干燥环境，温度28-32℃，湿度<60%', '喷洒阿维菌素，增加湿度，释放捕食螨'),
('p005', '黄瓜霜霉病', 'DISEASE', '由古巴假霜霉菌引起的病害，叶片出现多角形黄斑', '高湿环境，温度15-22℃，叶面有水滴', '喷洒霜脲氰，降低湿度，避免叶面积水'),
('p006', '黄瓜白粉病', 'DISEASE', '由白粉菌引起的病害，叶片表面有白色粉状物', '温暖干燥与高湿交替，温度16-24℃', '喷洒三唑酮，增加通风，合理灌溉'),
('p007', '棉铃虫', 'PEST', '幼虫蛀食果实，造成落果和烂果', '温度25-30℃，湿度适中', '释放赤眼蜂，喷洒甲维盐，灯光诱杀'),
('p008', '小麦条锈病', 'DISEASE', '由条形柄锈菌引起的病害，叶片出现黄色条状孢子堆', '温度10-15℃，高湿环境', '选用抗病品种，喷洒三唑酮'),
('p009', '小麦蚜虫', 'PEST', '吸食叶片汁液，分泌蜜露引发煤污病', '温暖干燥，温度18-22℃', '释放瓢虫，喷洒吡虫啉'),
('p010', '稻瘟病', 'DISEASE', '由稻瘟病菌引起的病害，叶片出现梭形病斑', '温度20-28℃，高湿多雨', '选用抗病品种，喷洒稻瘟灵');
```

### 3.5 巡检计划数据

```sql
INSERT INTO inspection_plan (id, name, cron_expression, active_hours_start, active_hours_end, is_active) VALUES
('ip001', '每日早间巡检', '0 0 6 * * ?', '06:00', '22:00', 1),
('ip002', '每日午间巡检', '0 0 12 * * ?', '06:00', '22:00', 1),
('ip003', '每日傍晚巡检', '0 0 18 * * ?', '06:00', '22:00', 1);

-- 巡检计划关联摄像头
INSERT INTO inspection_camera (plan_id, camera_id) VALUES
('ip001', 'c001'), ('ip001', 'c002'), ('ip001', 'c003'),
('ip002', 'c001'), ('ip002', 'c003'),
('ip003', 'c001'), ('ip003', 'c002'), ('ip003', 'c003'), ('ip003', 'c004');
```

---

## 4. ER关系说明

```
company ──────────────────────────────────────────────────────┐
    │                                                         │
    │ 1:N                                                     │
    ▼                                                         │
sys_user ─────────────────────────────────────────────────────┐
    │                                                         │
    │ 1:N                                                     │
    ▼                                                         │
report ──────┬──────► inference ──────► pest_info             │
    │        │                                               │
    │ 1:1    │ 1:N                                           │
    ▼        ▼                                               │
audit_record  prevention_plan ──► prevention_plan_version    │
    │                                                         │
    │ 1:1                                                     │
    ▼                                                         │
work_order ──────► work_order_history                        │
    │                                                         │
    │                                                         │
greenhouse ◄── grid ◄─── camera_grid ───► camera             │
    │                                                         │
    │ 1:N                                                     │
    ▼                                                         │
environment_data                                             │
    │                                                         │
    └─────────────────────────────────────────────────────────┘

inspection_plan ──► inspection_camera ──► camera
         │
         ▼
    inspection_log

daily_report ──► greenhouse

ai_conversation ──► ai_message
```

### 关键关联说明

| 关联关系 | 说明 |
|----------|------|
| report → sys_user | 上报用户 |
| report → grid | 上报所属网格 |
| inference → report | 识别结果关联上报 |
| inference → pest_info | 识别的病虫害类型 |
| audit_record → report | 审核记录关联上报 |
| prevention_plan → report | 防治方案关联上报 |
| work_order → inference | 工单关联识别结果 |
| camera_grid | 摄像头与网格多对多关系 |
| inspection_camera | 巡检计划与摄像头多对多关系 |

---

*文档编写完成。本文档将随项目开发进展持续更新。*
